...
static int canmakemove(creature const* cr, int dir, int flags) {
	...
	switch (floor) {
		...
		case Wall_Southeast: if (dir & (SOUTH | EAST)) return FALSE;
			break;
		case IceWall_Northwest: if (dir & (SOUTH | EAST)) return FALSE;
			break;
		case IceWall_Northeast: if (dir & (SOUTH | WEST)) return FALSE;
			break;
		case IceWall_Southwest: if (dir & (NORTH | EAST)) return FALSE;
			break;
		case IceWall_Southeast: if (dir & (NORTH | WEST)) return FALSE;
			break;
		case Beartrap:
		case CloneMachine:
...
static void initialhousekeeping(void) {
	...
	if (currentinput() >= CmdCheatNorth && currentinput() <= CmdCheatICChip) {
		switch (currentinput()) {
			case CmdCheatNorth: --yviewoffset();
				break;
			case CmdCheatWest: --xviewoffset();
				break;
			case CmdCheatSouth: ++yviewoffset();
				break;
			case CmdCheatEast: ++xviewoffset();
				break;
			case CmdCheatHome: xviewoffset() = yviewoffset() = 0;
				break;
			case CmdCheatKeyRed: ++possession(Key_Red);
				break;
			case CmdCheatKeyBlue: ++possession(Key_Blue);
				break;
			case CmdCheatKeyYellow: ++possession(Key_Yellow);
				break;
			case CmdCheatKeyGreen: ++possession(Key_Green);
				break;
			case CmdCheatBootsIce: ++possession(Boots_Ice);
				break;
			case CmdCheatBootsSlide: ++possession(Boots_Slide);
				break;
			case CmdCheatBootsFire: ++possession(Boots_Fire);
				break;
			case CmdCheatBootsWater: ++possession(Boots_Water);
				break;
			case CmdCheatICChip: if (chipsneeded()) --chipsneeded();
				break;
		}
		currentinput() = NIL;
		setnosaving();
...
static void preparedisplay(void) {
	...
	if (chip->moving) {
		switch (chip->dir) {
			case NORTH: yviewpos() += chip->moving;
				break;
			case WEST: xviewpos() += chip->moving;
				break;
			case SOUTH: yviewpos() -= chip->moving;
				break;
			case EAST: xviewpos() -= chip->moving;
				break;
		}
	}
...
static creature* get_value(void) {
